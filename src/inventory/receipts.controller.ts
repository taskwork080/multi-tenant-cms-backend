...
export class ReceiptsController {
  ...
  async create(@CurrentTenant() tenant: TenantDto, @Body() body: unknown) {
    ...
    return this.tdb.forTenant(tenant.id, async (tx) => {
      ...
      if (input.confirm) return this.confirmIn(tx, tenant.id, receipt.id);

      await this.inventory.writeActivity(tx, tenant.id, { action: "Drafted receipt", target: ref });
  ...
  async confirm(@CurrentTenant() tenant: TenantDto, @Param("id") id: string) {
    return this.tdb.forTenant(tenant.id, (tx) => this.confirmIn(tx, tenant.id, id));
  }
  ...
  private async confirmIn(tx: Db, tenantId: string, id: string) {
